...
def get_coordinates_for_assessment() -> list[tuple[float, float]]:
    """EPSG3857"""
    lats: Final[np.ndarray[float]] = np.arange(start=-84.5, stop=84.5, step=5)
    lons: Final[np.ndarray[float]] = np.arange(start=-179.5, stop=179.5, step=5)
    ...
    for lon, lat in itertools.product(lons, lats):
        if globe.is_land(lat=lat, lon=lon):
            coords_to_assess.append(tuple((lon, lat)))
